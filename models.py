...
class User:
    ...
    def find_busiest(cls):      
        ...
        with sqlite3.connect('todo.db') as con:
            cur = con.cursor()
            cur.execute("SELECT ROUND(AVG(rating), 2), name FROM grades GROUP BY name ORDER BY AVG(rating) DESC limit 5;")            
            #res = cur.fetchall()
    ...
    def find_busiest_discep(cls):   
        ...
        with sqlite3.connect('todo.db') as con:
            ...
            print(discep_list)
            discep = input('Введи назву предмета з списку вище для пошуку студента з найвищім середнім балом->>> ')
            cur.execute(f"SELECT  name, discipline, ROUND(AVG(rating),2) FROM grades WHERE discipline='{discep}' GROUP BY name ORDER BY AVG(rating) DESC limit 1;")            
            #res = cur.fetchall()
            for res in cur.fetchall():
...
class Groups_users:
    ...
    def get_all_statudents_from_group(cls):
        ...
        with sqlite3.connect('todo.db') as con:
            cur = con.cursor()
            cur.execute(f"SELECT name FROM groups_users WHERE group_us='{group}';")
...
class Discipline:
    ...
    def find_group_avg_discep(cls):   
        ...
        with sqlite3.connect('todo.db') as con:
            ...
            discep_list = "; ".join(f"{i[0]}" for i in discep_list)
            print(discep_list)
            discep = input('Введи назву предмета з списку вище для пошуку середньго балу по групам->>> ')
            cur.execute(f"SELECT grades.discipline, ROUND(AVG(grades.rating),2), groups_users.group_us FROM grades JOIN groups_users ON groups_users.name=grades.name WHERE grades.discipline='{discep}'  GROUP BY groups_users.group_us;")            
            #res = cur.fetchall()
            for res in cur.fetchall():
    ...
    def find_discipline_teacher(cls):      
        ...
        with sqlite3.connect('todo.db') as con:
            ...
            cur.execute(f"SELECT discipline FROM disciplines WHERE name='{teacher}';")            
...
class Grades:
    ...
    def find_rating_students_disce_of_group(cls):      
        ...
        with sqlite3.connect('todo.db') as con:
            ...
            discep_list = "; ".join(f"{i[0]}" for i in discep_list)
            print(discep_list)
            discep = input(f'Введи назву предмета з списку вище для отримання оцінок студентів группи {group}->>> ')
            cur.execute(f"SELECT grades.name, grades.rating, grades.date_gr FROM grades JOIN groups_users ON groups_users.name=grades.name WHERE groups_users.group_us='{group}' AND grades.discipline='{discep}';")            
            res = cur.fetchall()
            for i in res:
                print(f"{i[0]}, {i[1]}, {i[2]}")
    ...
    def find_AVG_teacher_disce(cls):      
        ...
        with sqlite3.connect('todo.db') as con:
            ...
            print(teacher_list)
            teacher = input('Введи вчителя зі списку вище для пошука предметів які він веде->>> ')
            cur.execute(f"SELECT ROUND(AVG(grades.rating),2)  FROM grades JOIN disciplines ON disciplines.discipline=grades.discipline WHERE disciplines.name='{teacher}' ;")            
            res = cur.fetchone()
    ...
    def find_disce_of_teacher(cls):      
        ...
        with sqlite3.connect('todo.db') as con:
            ...
            cur.execute(f"SELECT disciplines.name, disciplines.discipline  FROM disciplines  JOIN grades ON grades.discipline=disciplines.discipline WHERE grades.name='{students}' GROUP BY disciplines.name ;")            
            teacher_list = "; ".join(f"{i[0]}" for i in cur.fetchall())
            print(teacher_list)
            teacher = input(f'Введи вчителя зі списку вище для пошука предметів які він веде для {students}->>> ')

            cur.execute(f"SELECT grades.discipline  FROM grades  JOIN disciplines ON grades.discipline=disciplines.discipline WHERE grades.name='{students}' AND disciplines.name='{teacher}'  GROUP BY grades.discipline;")            
            res = "; ".join(f"{i[0]}" for i in cur.fetchall())
            print(f"Перелік курсів які читає {teacher} для студента {students} - {res}")